...
# =========================================================================
# SCRIPT: sast_simulacion_csrf.sh
# FUNCION: Analiza el riesgo de CSRF en un formulario de código provisto.
# =========================================================================
...
if [ -z "$1" ]; then
    ...
    echo " USO: ./scripts/sast_simulacion_csrf.sh /ruta/a/archivo.html"
    exit 1
fi

RUTA_ARCHIVO="$1"
FECHA_HORA=$(date +"%Y%m%d_%H%M%S")
ARCHIVO_REPORTE="resultados/REPORTE_SAST_CSRF_${FECHA_HORA}.txt"
...
echo " "
echo "--- 🔎 Ejecutando prueba de 'SAST_CSRF' en archivo: ${RUTA_ARCHIVO} ---"

# --- 2. Leer y Codificar el Código Fuente del Archivo en Base64 (CORRECCIÓN) ---
CODIGO_FUENTE_BASE64=$(cat "${RUTA_ARCHIVO}" | base64)

echo "🤖 Enviando contenido del archivo codificado en Base64 para análisis de CSRF..."

# --- 3. Definir y Ejecutar el Prompt a Gemini con Base64 ---
PROMPT="Eres un experto en ciberseguridad que genera reportes ejecutivos e intuitivos. El siguiente código está codificado en Base64. Primero, **DECODIFICA** el contenido. Luego, analiza el código decodificado buscando un riesgo de **Cross-Site Request Forgery (CSRF)**, específicamente la ausencia de un token de seguridad en formularios de acción crítica (POST). Tu respuesta debe ser una lista de chequeo concisa. Indica: 1) El Riesgo (ALTO, BAJO, o NO DETECTADO). 2) La Explicación (qué permite hacer la falla o por qué es seguro). 3) La Solución (un solo paso de refactorización de código añadiendo un token anti-CSRF). La respuesta DEBE empezar con el encabezado # CHECKLIST DE RIESGO SAST - CSRF. Responde únicamente con el reporte formateado de lista concisa en lenguaje natural y humanizado:"
...
echo " "
echo "---------------------------------------------------------"
echo "✅ Prueba SAST de CSRF finalizada. Reporte guardado."