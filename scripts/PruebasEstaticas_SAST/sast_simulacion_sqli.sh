...
# =========================================================================
# SCRIPT: sast_simulacion_sqli.sh
# FUNCION: Analiza el riesgo de SQL Injection en un archivo de código provisto.
# =========================================================================
...
if [ -z "$1" ]; then
    ...
    echo " USO: ./scripts/sast_simulacion_sqli.sh /ruta/a/archivo.php"
    exit 1
fi

RUTA_ARCHIVO="$1"
FECHA_HORA=$(date +"%Y%m%d_%H%M%S")
ARCHIVO_REPORTE="resultados/REPORTE_SAST_SQLI_${FECHA_HORA}.txt"
...
echo " "
echo "--- 🔎 Ejecutando prueba de 'SAST_SQLI' en archivo: ${RUTA_ARCHIVO} ---"

# --- 2. Leer y Codificar el Código Fuente del Archivo en Base64 (CORRECCIÓN) ---
CODIGO_FUENTE_BASE64=$(cat "${RUTA_ARCHIVO}" | base64)

echo "🤖 Enviando contenido del archivo codificado en Base64 para análisis de SQL Injection..."

# --- 3. Definir y Ejecutar el Prompt a Gemini con Base64 ---
PROMPT="Eres un experto en ciberseguridad que genera reportes ejecutivos e intuitivos. El siguiente código está codificado en Base64. Primero, **DECODIFICA** el contenido. Luego, analiza el código decodificado para identificar riesgos de **Inyección SQL** (ej: uso de variables de usuario sin sanitizar directamente en consultas DB). Tu respuesta debe ser una lista de chequeo concisa. Indica: 1) El Riesgo (ALTO, BAJO, o NO DETECTADO). 2) La Explicación (qué permite hacer la falla o por qué es seguro). 3) La Solución (un solo paso de refactorización de código usando consultas preparadas). La respuesta DEBE empezar con el encabezado # CHECKLIST DE RIESGO SAST - SQLI. Responde únicamente con el reporte formateado de lista concisa en lenguaje natural y humanizado:"
...
echo " "
echo "---------------------------------------------------------"
echo "✅ Prueba SAST de SQLI finalizada. Reporte guardado."